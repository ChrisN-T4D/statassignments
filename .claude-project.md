# Statistics Teaching Site - Project Documentation

## Project Overview

A Vue 3 + PocketBase web application for teaching introductory statistics in psychology with pseudonymous student tracking. Supports multiple classes (PSYC 4213 Statistics, PSYC 4223 Research Methods) with software-specific content (jamovi, SPSS, R, Stata, Excel).
...
**Fields:**
- `semester` (relation → semesters, required)
...
**Fields:**
- `class` (relation → classes) - NEW: links module to specific class
- `topic_id` (text, required) - e.g., "correlation"
- `title` (text, required)
- `description` (text)
- `order` (number)
- `software_type` (text) - "jamovi", "spss", "r", "stata", "excel", or "all"
- `semester` (relation → semesters) - can be empty for "available to all"
...
**Fields:**
- `module` (relation → modules, required)
- `title` (text, required)
- `description` (text)
...
- `user` (relation → users) - linked when student claims key
...
- `user` (relation → users) - redundant with student_key for queries
- `module` (relation → modules, required)
...
├── src/
│   ├── components/
│   │   ├── ClassHeader.vue              # Class navigation header
│   │   ├── ClassSelector.vue            # Dropdown for class selection
│   │   ├── ModuleItem.vue               # Individual module display
│   │   ├── PracticeItem.vue             # Practice question display
│   │   ├── ProgressTracker.vue          # Student progress visualization
│   │   ├── RosterUpload.vue             # CSV roster import
│   │   ├── SoftwareInstructions/
│   │   │   ├── JamoviInstructions.vue
│   │   │   ├── SPSSInstructions.vue
│   │   │   ├── RInstructions.vue
│   │   │   ├── StataInstructions.vue
│   │   │   └── ExcelInstructions.vue
│   │   └── TopicCard.vue                # Topic selection card
│   │
│   ├── composables/
│   │   ├── useAuth.js                   # Authentication logic
│   │   ├── useClasses.js                # Class fetching & selection (slug support)
│   │   ├── useModules.js                # Module/item fetching & filtering
│   │   ├── useAttempts.js               # Attempt logging & progress tracking
...
│   │   ├── ClassHome.vue                # Class overview (topics list)
│   │   ├── TopicView.vue                # Topic modules list
│   │   ├── ModuleView.vue               # Module practice interface
│   │   ├── Practice.vue                 # General practice (random/by topic)
│   │   ├── Profile.vue                  # Student progress dashboard
│   │   └── InstructorDashboard.vue      # Analytics & roster management
│   │
│   ├── lib/
...
│   ├── software-instructions.js         # Software-specific instructions
│   └── classes.js                       # Class definitions
│
├── scripts/
│   ├── import-seed-data.js              # Import semesters/modules/items
│   ├── setup-classes.js                 # Create classes & link modules
│   ├── add-class-slugs.js               # Add slug field to classes
│   ├── import-classes.js                # Import classes from JSON
│   ├── test-connection.js               # Verify PocketBase connection
│   └── classes-seed-data.json           # Class seed data
│
├── pocketbase-schema.json               # Complete PB schema definition
├── pocketbase-schema.md                 # Human-readable schema docs
├── SETUP.md                             # Detailed setup guide
├── QUICK-FIX.md                         # Common issues & fixes
├── MANUAL-IMPORT-GUIDE.md               # Fallback manual import steps
...
**Status:** ✅ Complete (1 semester, 11 Statistics modules, 23 items imported)
...
**Status:** ✅ Complete (2 classes created, 11 modules linked to PSYC 4213, 12 modules linked to PSYC 4223)
...
**Status:** ✅ Complete (slug field added, classes updated)
...
**Status:** ✅ Complete (12 modules created and linked to PSYC 4223)
...
✓ Collection 'modules' exists
✓ Collection 'items' exists
✓ Collection 'attempts' exists
✓ Collection 'roster' exists
...
2. Export schema: Collections → Export
...
- **Email:** christopher.neu12@gmail.com
